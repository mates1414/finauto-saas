...
from ..models import Job, Snapshot
...
from finauto.schemas import CompanyFinancials, MarketData
            ...
            if stream:
                with client.messages.stream(**kwargs) as s:
                    for text in s.text_stream:
                        tokens.append(text)
                        if loop.is_running():
                            asyncio.run_coroutine_threadsafe(pubsub.publish(self.job_id, text), loop)
                        else:
    ...
    try:
        ...
        if snapshot:
            ...
            if snapshot.financials_json:
                original_fin = CompanyFinancials.model_validate_json(snapshot.financials_json)
        ...
        ctx = build_context(fin_edited, computed, original=original_fin, market=market_data)
        ...
        if ungrounded:
            ...
            job.error = json.dumps({"warnings": [f"Ungrounded figure: {num}" for num in ungrounded]})
        db.commit()
